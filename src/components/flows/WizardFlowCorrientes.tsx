...
import { flujoEnsayoMachine } from "../../utils/flowCorrientesFuga";
...
export default function WizardFlowTest() {
  ...
  const [parrafoON, setParrafoON] = useState(false);
  const [textoParrafo, setTextoparrafo] = useState("");
  ...
  const [showPopupRepetirFichaEquipo, setShowPopupRepetirFichaEquipo] =
    useState(false);
  ...
  useEffect(() => {
    ...
    send({
      type: "SET_TIPO_PARTES_APLICABLES",
      value: datosEnsayo.tipoParteAplicable,
    });
  ...
  useEffect(() => {
    ...
    if (state.matches("repetirFichaEquipo")) {
      setShowPopupRepetirFichaEquipo(true);
      setMedicionON(false);
      setFiguraON(false);
    }
    if (state.matches("textoYaIncluida")) {
      setTextoFichaON(false);
      setDisabledNext(false);
      setText("Tipo de partes aplicables: B");
      setSubtitle("Corriente de fuga de partes aplicables");
      setParrafoON(true);
      setTextoparrafo(
        "La medición de corrientes de fuga de las partes aplicables del tipo B se incluye en las corrientes de fuga del aparato. Solo es necesaria si lo especifica el fabricante."
      );
    }
  }, [state, measurements]);
  ...
  return (
    <div>
      ...
      <h2>
        {state.context.claseProteccion} - Alimentacion:{" "}
        {state.context.tipoAlimentacion} - Partes:{" "}
        {state.context.tipoPartesAplicables}
      </h2>

      <DoubleCard text={text} subtitle={subtitle}></DoubleCard>
      {parrafoON && <Parrafo>{textoParrafo}</Parrafo>}
      {textoFichaON && (
      ...
      <PopupConfirm
        isOpen={showPopupRepetirFichaEquipo}
        title={
        ...
        onConfirm={() => {
          setShowPopupRepetirFichaEquipo(false);
          send({ type: "SI" });
        }}
        confirmText="Sí"
        onCancel={() => {
